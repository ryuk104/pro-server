...
router.get ("/buildDescendantCountMap", (req, res) => {
...
router.get ("/getNeighbors", (req, res, note, depth) => {
    if (depth === 0) {
        return [];
    }
...
router.post ("/getLinkMap", (req, res) => {
...
router.post ("/getTreeMap", (req, res) => {
    const mapRootNote = becca.getNote(req.params.noteId);
    // if the map root itself has ignore (journal typically) then there wouldn't be anything to display so
    // we'll just ignore it
    const ignoreExcludeFromNoteMap = mapRootNote.hasLabel('excludeFromNoteMap');
...
router.get ("/removeImages", (req, res, document) => {
...
router.get ("/findExcerpts", (req, res, sourceNote, referencedNoteId) => {
...
router.get ("/getBacklinks", (req, res) => {
